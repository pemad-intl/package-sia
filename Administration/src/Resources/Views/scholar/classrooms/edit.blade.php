...
@section('content')
    ...
    <div class="row">
        <div class="col-md-8">
            <div class="card mb-4">
                <div class="card-body">
                    <form class="form-block" action="{{ route('administration::scholar.classrooms.update', ['classroom' => $classroom->id, 'next' => request('next', route('administration::scholar.classrooms.index', ['academic' => request('academic', $classroom->semester_id)]))]) }}" method="POST"> @csrf @method('PUT')
                            ...
                            <div class="col-md-5">
                                <select class="form-control @error('level_id') is-invalid @enderror" name="level_id" id="level_id">
                                    @foreach (getGrade()->levels as $level)
                                        <option value="{{ $level->id }}" @if (old('level_id', $classroom->level_id) == $level->id) selected @endif>{{ $level->kd . ' - ' . $level->name }}</option>
                                    @endforeach
                                </select>
                                @error('level_id')
                        ...
                        <div class="form-group required row mb-3">
                            ...
                            <div class="col-md-4">
                                <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" id="name" placeholder="Nama rombel" value="{{ old('name', $classroom->name) }}">
                                @error('name')
                                    <small class="invalid-feedback"> {{ $message }} </small>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-7">
                                <select class="form-control select-2 @error('room_id') is-invalid @enderror" name="room_id" id="room_id">
                                    ...
                                    @foreach ($rooms as $room)
                                        <option value="{{ $room->id }}" @if (old('room_id', $classroom->room_id) == $room->id) selected @endif>{{ $room->kd . ' - ' . $room->name }}</option>
                                    @endforeach
                                </select>
                                @error('room_id')
                                    <small class="invalid-feedback"> {{ $message }} </small>
                            ...
                            <div class="col-md-7">
                                <select class="form-control select-2 @error('major_id') is-invalid @enderror" name="major_id" id="major_id">
                                    ...
                                    @foreach ($acsem->majors as $major)
                                        <option value="{{ $major->id }}" @if (old('major_id', $classroom->major_id) == $major->id) selected @endif>{{ $major->name }}</option>
                                    @endforeach
                                </select>
                                @error('major_id')
                                    <small class="invalid-feedback"> {{ $message }} </small>
                            ...
                            <div class="col-md-7">
                                <select class="form-control select-2 @error('superior_id') is-invalid @enderror" name="superior_id" id="superior_id">
                                    ...
                                    @foreach ($acsem->superiors as $superior)
                                        <option value="{{ $superior->id }}" @if (old('superior_id', $classroom->superior_id) == $superior->id) selected @endif>{{ $superior->name }}</option>
                                    @endforeach
                                </select>
                                @error('superior_id')
                                    <small class="invalid-feedback"> {{ $message }} </small>
                            ...
                            <div class="col-md-6">
                                <select class="form-control select-2 @error('supervisor_id') is-invalid @enderror" name="supervisor_id" id="supervisor_id">
                                    ...
                                    @foreach ($supervisors as $supervisor)
                                        <option value="{{ $supervisor->id }}" @if (old('supervisor_id', $classroom->supervisor_id) == $supervisor->id) selected @endif>{{ $supervisor->user->name }}</option>
                                    @endforeach
                                </select>
                                @error('supervisor_id')
                                    <small class="invalid-feedback"> {{ $message }} </small>
                        ...
                        <div class="form-group row mb-0">
                            <div class="col-md-8 offset-md-3">
                                ...
                                <a class="btn btn-secondary" href="{{ request('next', route('administration::scholar.classrooms.index', ['academic' => $classroom->semester_id])) }}">Kembali</a>
                            </div>
                        </div>
                    </form>
                </div>
                    ...
                    <div class="input-group w-100">
                        <select name="academic" class="form-control" disabled>
                            @foreach ($acsems as $_acsem)
                                <option value="{{ $acsem->id }}" @if (request('academic', $acsem->id) == $_acsem->id) selected @endif>{{ $acsem->full_name }}</option>
                            @endforeach
                        </select>
                        <div class="input-group-append">
                            <button class="btn btn-primary disabled">Tetapkan</button>
                        </div>
                    </div>
                </div>
            </div>
