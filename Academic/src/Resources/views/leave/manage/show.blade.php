        ...
        <div class="col-xl-8">
            <div class="card border-0">
                <div class="card-body d-flex align-items-center justify-content-between">
                    ...
                    @if (!$leave->trashed())
                        <a class="btn btn-soft-success btn-sm rounded px-2 py-1" href="{{ route('academic::leave.print', ['leave' => $leave->id]) }}" target="_blank"><i class="mdi mdi-printer-outline"></i> <span class="d-none d-sm-inline">Cetak dokumen (.pdf)</span></a>
                    @endif
                </div>
                            ...
                            <div class="row gy-2 @if (!$loop->last) mb-4 @endif">
                                ...
                                <div class="col-md-6">
                                    @if ($approvable->userable?->is($employee?->position) && !$leave->trashed())
                                        <form class="form-block" action="{{ route('academic::leave.manage.update', ['approvable' => $approvable->id, 'next' => request('next', route('academic::leave.manage.index'))]) }}" method="post"> @csrf @method('PUT')
                                            <div class="mb-3">
