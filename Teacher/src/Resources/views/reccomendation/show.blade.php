...
@section('title', 'Catatan kenaikan kelas - ' . $cls->name)
    ...
    <div class="row">
        <div class="col-md-8">
            ...
            <div class="card">
                ...
                <form class="form-block form-confirm" action="{{ route('teacher::recommended', ['classroom' => $cls->id, 'next' => url()->current()]) }}" method="POST"> @csrf @method('PUT')
                                ...
                                @forelse($cls->stsems as $stsem)
                                    <tr>
                                        ...
                                        <td class="py-1" style="min-width: 200px;">
                                            <textarea class="form-control" name="value[{{ $stsem->id }}][recommendation_note]" required placeholder="Rekomendasi Jenjang">{{ old("value.{$stsem->id}.recommendation_note", $stsem->reportEval->first()->recommendation_note) }}</textarea>
                                        </td>
                                        <td>
                                            <select class="form-select" name="value[{{ $stsem->id }}][grade]">
                                                <option {{  $stsem->reportEval->first()->grade == 10 ? 'selected' : '' }} value="10">10</option>
                                                <option {{  $stsem->reportEval->first()->grade == 11 ? 'selected' : '' }} value="11">11</option>
                                                <option {{  $stsem->reportEval->first()->grade == 12 ? 'selected' : '' }} value="12">12</option>
                                            </select>
                                        </td>
                                    </tr>
                                    @if ($loop->last)
