    ...
    <div class="row">
        <div class="col-md-7 col-lg-8">
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <form action="{{ route('counseling::manage.counseling.categories.index') }}" method="GET">
                        ...
                        <div class="input-group">
                            ...
                            <div class="input-group-append">
                                <a class="btn btn-outline-secondary" href="{{ route('counseling::manage.counseling.categories.index') }}"><i class="mdi mdi-refresh"></i></a>
                            ...
                            @forelse($categories as $category)
                                <tr @if ($category->trashed()) class="text-muted bg-light" @endif>
                                    ...
                                    <td nowrap class="py-2 text-right align-middle">
                                        @if ($category->trashed())
                                            <form class="d-inline form-block form-confirm" action="{{ route('counseling::manage.counseling.categories.restore', ['category' => $category->id]) }}" method="POST"> @csrf @method('PUT')
                                                <button class="btn btn-primary btn-sm" data-toggle="tooltip" title="Pulihkan"><i class="mdi mdi-restore"></i></button>
                                            </form>
                                            <form class="d-inline form-block form-confirm" action="{{ route('counseling::manage.counseling.categories.kill', ['category' => $category->id]) }}" method="POST"> @csrf @method('DELETE')
                                        ...
                                        @else
                                            <a class="btn btn-warning btn-sm" href="{{ route('counseling::manage.counseling.categories.edit', ['category' => $category->id]) }}" data-toggle="tooltip" title="Ubah"><i class="mdi mdi-pencil-outline"></i></a>
                                            <form class="d-inline form-block form-confirm" action="{{ route('counseling::manage.counseling.categories.destroy', ['category' => $category->id]) }}" method="POST"> @csrf @method('DELETE')
        ...
        <div class="col-md-5 col-lg-4">
            <div class="card">
                <div class="card-body">
                    ...
                    <small class="text-muted text-uppercase font-weight-bold">Jumlah kategori konseling</small>
                </div>
            </div>
            <div class="card">
                <div class="card-header">
