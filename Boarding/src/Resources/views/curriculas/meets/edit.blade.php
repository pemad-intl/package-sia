							...
							<div class="col-md-5">
								<select class="form-control @error('teacher_id') is-invalid @enderror" name="teacher_id" id="teacher_id" required>
									...
									@foreach($teachers as $teacher)
										<option value="{{ $teacher->id }}" @if(old('teacher_id', $meet->teacher_id) == $teacher->id) selected @endif>{{ $teacher->full_name }}</option>
									@endforeach
								</select>
								@error('teacher_id')
									<small class="invalid-feedback"> {{ $message }} </small>
							...
							<div class="col-md-5">
								<div class="btn-group-toggle" data-toggle="buttons">
									@foreach(\Modules\Academic\Models\AcademicSubjectMeet::$colors as $color)
										<label class="btn btn-outline-{{ $color }} rounded-circle @if($meet->props && $color == $meet->props->color) active @endif" style="width: 26pt; height: 26pt;">
											<input type="radio" name="props[color]" autocomplete="off" value="{{ $color }}" @if($meet->props && $color == $meet->props->color) checked @endif>
										</label>
									@endforeach
								</div>
							</div>
