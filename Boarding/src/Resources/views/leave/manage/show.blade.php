...
@section('content')
    <div class="d-flex align-items-center mb-4">
        ...
        <div class="ms-4">
            ...
            <div class="text-muted">Kamu dapat menyetujui/menolak/merevisi pengajuan izin yang diajukan siswa!</div>
        </div>
    </div>
    @if ($leave->trashed())
        ...
        <div class="col-xl-8">
            <div class="card border-0">
                <div class="card-body d-flex align-items-center justify-content-between">
                    ...
                    @if (!$leave->trashed())
                        <a class="btn btn-soft-success btn-sm rounded px-2 py-1" href="{{ route('boarding::leave.print', ['leave' => $leave->id]) }}" target="_blank"><i class="mdi mdi-printer-outline"></i> <span class="d-none d-sm-inline">Cetak dokumen (.pdf)</span></a>
                    @endif
                </div>
                        ...
                        @foreach ($leave->approvables->sortBy('level') as $approvable)
                            <div class="row gy-2 @if (!$loop->last) mb-4 @endif">
                                <div class="col-md-6">
                                    ...
                                    <strong>{{ in_array(strtolower($approvable->userable->getApproverLabel()), ['guru', 'humas']) ? 'Wali Kelas' : $approvable->userable->getApproverLabel() }}</strong>
                                </div>
                                <div class="col-md-6">
                                    @if ($approvable->userable->is($employee->position) && !$leave->trashed())
                                        <form class="form-block" action="{{ route('boarding::leave.manage.update', ['approvable' => $approvable->id, 'next' => request('next', route('boarding::leave.manage.index'))]) }}" method="post"> @csrf @method('PUT')
                                            <div class="mb-3">
            ...
            <div class="col-xl-4">
                <div class="card border-0">
                    <div class="card-body">
                        <i class="mdi mdi-account-box-multiple-outline"></i> Detail siswa
                    </div>
                    <div class="list-group list-group-flush border-top">
                        @foreach (array_filter([
                'Nama Siswa' => $leave->student->user->name,
                'NIS' => $leave->student->nis ?: '-',
                'Pengasuh' => $parentBoard->employee->user->name,
            ]) as $label => $value)
