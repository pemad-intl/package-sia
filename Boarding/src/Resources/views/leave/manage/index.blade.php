                    ...
                    <div class="col-xl-8">
                        <div class="card border-0">
                            ...
                            <div class="card-body border-top border-light">
                                <form class="form-block row gy-2 gx-2" action="{{ route('boarding::leave.manage.index') }}" method="get">
                                    ...
                                    <div class="col-auto">
                                        <a class="btn btn-light" href="{{ route('boarding::leave.manage.index', request()->only('pending')) }}"><i class="mdi mdi-refresh"></i> <span class="d-sm-none">Reset</span></a>
                                    </div>
                                    <div class="col-auto">
                                                ...
                                                @unless ($leave->trashed())
                                                    ...
                                                    <div class="dropstart d-inline">
                                                        ...
                                                        <ul class="dropdown-menu border-0 shadow">
                                                            <li><a class="dropdown-item" href="{{ route('boarding::leave.manage.show', ['leave' => $leave->id, 'next' => request('next')]) }}"><i class="mdi mdi-eye-outline me-1"></i> Lihat detail</a></li>
                                                            @if (isset($leave->attachment) && Storage::exists($leave->attachment))
                                                            ...
                                                            @endif
                                                            <li><a class="dropdown-item" href="{{ route('boarding::leave.print', ['leave' => $leave->id]) }}" target="_blank"><i class="mdi mdi-printer-outline me-1"></i> Cetak dokumen (.pdf)</a></li>
                                                        </ul>
                                                    </div>
                                                @endunless
                                            </td>
                                                            ...
                                                            <table class="table-borderless table-hover table-sm mb-0 table align-middle">
                                                                ...
                                                                <tbody>
                                                                   @foreach ($leave->approvables->sortBy('level') as $approvable)
                                                                        <tr>
                                                                            <td class="small {{ $approvable->cancelable ? 'text-danger' : 'text-muted' }}">
                                                                                {{ ucfirst($approvable->type) }} #{{ $approvable->level }}
                                                                            </td>
                                                                            <td @if ($loop->last) class="border-0" @endif>
                                                                                <div class="badge bg-{{ $approvable->result->color() }} fw-normal text-white">
                                                                                    <i class="{{ $approvable->result->icon() }}"></i> {{ $approvable->result->label() }}
                                                                                </div>
                                                                            </td>
                            ...
                            <div class="col-xl-4">
                                <div class="card border-0">
                                    ...
                                    <div class="list-group list-group-flush border-top">
                                        <a class="list-group-item list-group-item-action text-danger" href="{{ route('boarding::leave.manage.index', ['pending' => !request('pending')]) }}"><i class="mdi mdi-progress-clock"></i> {{ request('pending') == 1 ? 'Tampilkan semua pengajuan' : 'Hanya tampilkan pengajuan yang tertunda' }}</a>
                                    </div>
                                </div>
                                <a class="btn w-100 d-flex justify-content-between align-items-center bg-white py-3 text-start" style="cursor: pointer;" href="{{ route('boarding::leave.submission.create', ['next' => url()->full()]) }}">
